...
var digWAObj;
var digALHost;
if (window.location.protocol == "https:") digALHost = "https://a2.disneyinternational.com/analytics/";
else digALHost = "http://a2.disneyinternational.com/analytics/";
...
function debug(str) {
    ...
    if (typeof(console) != "undefined" && console.debug)
        console.debug("ANALYTICS DEBUG: " + str);
}
...
function digWAObjSubmit() {
    ...
    if (digWAObj.useWebService) {
        ...
        var report = digWAObj;
        if (!disableOldScode) digIncludeJavaScript(digALHost + 'tags/tagsv2.php?site=' + digWAUrlSite + '&uri=' + digWAUrl + "&hash=" + encodeURIComponent(window.location.hash) + "&search=" + encodeURIComponent(window.location.search));
    } else {
        digIncludeJavaScript(digALHost + 'scripts/' + digWAObj.urlSite + '.js');
    }

    if (!disableOldScode) digIncludeJavaScript(digALHost + 'lib/' + digWALibVersion + '/s_code.js');
    if (typeof(window.s_omni) === "undefined") window.s_omni = {};
